...
void input_para(string name)
{
    ...
    string subName = part_out_name_base+ "/Particle_parallel2_";
...
{
    string vtk_name = part_out_name_base+"/Particle_parallel2_/"+vtk_out_name_base + "_" + to_string(vtk_count) + ".vtk";
    fstream file;
...
{
    string out_name = part_out_name_base+"/Particle_parallel2_/"+part_out_name_base + "_" + to_string(out_count) + ".out";
    fstream file;
...
int main( int argc, char *argv[] )
{
    ...
    update_cell<data_type><<<domain.block_size,domain.thread_size>>>(elements,N,domain, d_cell_arr); // update cell array with particle pisition
    ...
    string result_name = part_out_name_base+"/Particle_parallel2_/result_time.txt";
    fstream file1;
  ...
  while(count <= int(std::round(time_end/timestep_length)))
    {
     ...
     // ************************ write out vtk and out **********************
      ...
      // ************************ Calculate distance **********************
        ...
        checkError(cudaMemPrefetchAsync(elements, N*sizeof(particle<data_type>), device, NULL));
        checkError(cudaMemPrefetchAsync(d_cell_arr, domain.cell_size*sizeof(int), device, NULL));
        //checkError(cudaDeviceSynchronize());*/
        ...
        init_particle<data_type><<<domain.block_size,domain.thread_size>>>(elements,N); // CHANGE
      ...
      // ************************ calculate force **********************  
       /* checkError(cudaGetDevice(&device));
        ...
        main_kernel_force<data_type><<<domain.block_size,domain.thread_size>>>(elements,timestep_length,epsilon,sigma,N,domain,d_cell_arr, gpu_par, gpu_count);
       ...
       /* checkError(cudaGetDevice(&device));
        ...
        main_host_force<data_type>(elements,timestep_length,epsilon,sigma,N,domain,d_cell_arr, cpu_par, cpu_count);
    ...
    std::cout << "elapsed time Unified " << elapsed_seconds.count() << "s\n";
    ...
    file<<"Unified Memory Variable_Block :"<<elapsed_seconds.count() << "\t"<<domain.cpu_x<<"\n";
    file.close();
    file1.close();
}
