...
void input_para(string name)
{
    ...
    string subName = part_out_name_base+ "/Particle_parallel4_";
...
template <typename T>
__global__ void init_particle (particle<T> *elements, int num_par)
{
...
template <typename T>
__host__ tuple<int,int> init_cpu_gpu(particle<T> *elements, Domain<T> domain,int num_par, int *cpu_par, int *gpu_par)
{
    int cpu_count = 0, gpu_count = 0;
    for(int i=0; i<num_par; ++i)
    {
        int x_cell = (elements[i].x-domain.x_min)/domain.x_len;
        cpu_par[i] = -1;
        gpu_par[i] = -1;
    ...
    return make_tuple(cpu_count,gpu_count);
}
...
template <typename T>
__device__ void update_cell_calc (particle<T> *elements, int id, Domain<T> domain, int *cell_arr)
{
...
template <typename T>
__global__ void update_cell (particle<T> *elements, int num_par, Domain<T> domain, int *cell_arr)
{
    int id = threadIdx.x + blockIdx.x * blockDim.x;
    
    while(id<num_par)
    {
        update_cell_calc<T>(elements, id, domain, cell_arr);
        id = id + blockDim.x * gridDim.x;
    }
...
template <typename T>
__global__ void main_kernel_force (particle<T> *elements, T delta_t, T epsilon, T sigma, int num_par, Domain<T> domain, int * cell_arr, int *gpu_par, int gpu_count )
{
    int idg = threadIdx.x + blockIdx.x * blockDim.x;
...
template <typename T>
__host__ void main_host_force (particle<T> *elements, T delta_t, T epsilon, T sigma, int num_par, Domain<T> domain, int * cell_arr, int *cpu_par, int cpu_count )
{
...
{
    string vtk_name = part_out_name_base+"/Particle_parallel4_/"+vtk_out_name_base + "_" + to_string(vtk_count) + ".vtk";
    fstream file;
...
{
    string out_name = part_out_name_base+"/Particle_parallel4_/"+part_out_name_base + "_" + to_string(out_count) + ".out";
    fstream file;
...
/*void cpu_gpu_copy( particle<T> *elements, particle<T> *d_elements, int num_par, int *gpu_par)
{
    int i=0;
    
    while(gpu_par[i] != -1)
    {
        elements[gpu_par[i]] = d_elements[gpu_par[i]];
        i++;
    }
}
*/
...
{
    data_type t = 0;
    int N,i=0,count = 0, *d_cell_arr, *cpu_par, *gpu_par,*cpu_cell_arr;
    double negative_ratio = 1, positiv_ratio = 1;
    bool  file_val = true;
    string::size_type sz; 
    string testline, name;
    input_para(argv[1]);
    tuple<int,int> cpu_gpu_count;
    int cpu_count = 0;
    int gpu_count = 0;
    ...
    particle<data_type>*d_elements;
    //checkError(cudaMallocManaged(&elements,N*sizeof(particle<data_type>))); // create object for particle structure in device
    
    particle<data_type> * elements = new particle<data_type>[N];
    particle<data_type> * temp_elem = new particle<data_type>[N];
    ...
    checkError(cudaMalloc(&d_elements,N*sizeof(particle<data_type>))); // create object for particle structure in device
	checkError(cudaMemcpy(d_elements,&elements[0],N*sizeof(particle<data_type>),cudaMemcpyHostToDevice)); // copy particel object from host to device
    
    
    checkError(cudaMalloc((void**)&d_cell_arr,domain.cell_size*sizeof(int))); // // cell array initializatio in device
    cpu_cell_arr = ( int*)malloc(domain.cell_size*sizeof(int));
    init_cell<data_type><<<domain.block_size,domain.thread_size>>>(domain, d_cell_arr); // initializing cell array with -1
    cpu_gpu_count = init_cpu_gpu<data_type>(elements,domain,N,cpu_par,gpu_par);
    cpu_count = get<0>(cpu_gpu_count);
    gpu_count = get<1>(cpu_gpu_count);
    checkError(cudaDeviceSynchronize());
    
    update_cell<data_type><<<domain.block_size,domain.thread_size>>>(d_elements,N,domain, d_cell_arr); // update cell array with particle pisition
    checkError(cudaPeekAtLastError());
    checkError(cudaDeviceSynchronize());
    
     
    init_force<data_type><<<domain.block_size,domain.thread_size>>>(d_elements,epsilon,sigma, N, domain, d_cell_arr); // calculate initial force
    checkError(cudaPeekAtLastError());
    checkError(cudaDeviceSynchronize());
    
    
    
    string result_name = part_out_name_base+"/Particle_parallel4_/result_time.txt";
    fstream file1;
  ...
  while(count <= int(std::round(time_end/timestep_length)))
    {
     ...
     // ************************ write out vtk and out **********************
      ...
      // ************************ Calculate distance **********************
        ...
        auto start_dist = std::chrono::system_clock::now();
        main_kernel_dist<data_type><<<domain.block_size,domain.thread_size>>>(d_elements,timestep_length,epsilon,sigma,N,domain,d_cell_arr);
        checkError(cudaPeekAtLastError());
        checkError(cudaDeviceSynchronize());
        
        init_particle<data_type><<<domain.block_size,domain.thread_size>>>(d_elements,N); // CHANGE
        init_cell<data_type><<<domain.block_size,domain.thread_size>>>(domain, d_cell_arr);
        checkError(cudaPeekAtLastError());
        checkError(cudaDeviceSynchronize());
        auto end_dist = std::chrono::system_clock::now();
        std::chrono::duration<double> elapsed_seconds_dist = end_dist-start_dist;
        ...
        checkError(cudaMemcpy(&elements[0],d_elements, N*sizeof(particle<data_type>),cudaMemcpyDeviceToHost));
        checkError(cudaMemcpy(&cpu_cell_arr[0],d_cell_arr, domain.cell_size*sizeof(int),cudaMemcpyDeviceToHost));
        checkError(cudaDeviceSynchronize());
        
        auto end_up = std::chrono::system_clock::now();
        std::chrono::duration<double> elapsed_seconds_up = end_up-start_up;
        
    
        auto start_cpu_for = std::chrono::system_clock::now();
        
        cpu_gpu_count = init_cpu_gpu<data_type>(elements,domain,N,cpu_par,gpu_par);
        cpu_count = get<0>(cpu_gpu_count);
        gpu_count = get<1>(cpu_gpu_count);
        
        auto end_cpu_for = std::chrono::system_clock::now();
        std::chrono::duration<double> elapsed_seconds_cpu_for = end_cpu_for-start_cpu_for;
      ...
      // ************************ calculate force **********************  
        ...
        domain.block_size = gpu_count/domain.thread_size +1;
        main_kernel_force<data_type><<<domain.block_size,domain.thread_size>>>(d_elements,timestep_length,epsilon,sigma,N,domain,d_cell_arr, gpu_par, gpu_count);
        checkError(cudaEventRecord(stop));
        
        
        auto start_cpu = std::chrono::system_clock::now();
        main_host_force<data_type>(elements,timestep_length,epsilon,sigma,N,domain,cpu_cell_arr, cpu_par, cpu_count);
        ...
        checkError(cudaMemcpy(&temp_elem[0],d_elements, N*sizeof(particle<data_type>),cudaMemcpyDeviceToHost));
        
        
        
        
        
        for(int i_cpy = 0; i_cpy < N; i++)
        {
           if(gpu_par[i_cpy] != -1)
           {   
                elements[gpu_par[i_cpy]] = temp_elem[gpu_par[i_cpy]];
                i_cpy++;
            }
            else
            break;
        }
        checkError(cudaMemcpy(d_elements,&elements[0],N*sizeof(particle<data_type>),cudaMemcpyHostToDevice));
       ...
       if(time_ratio<1)
        ...
        domain.block_size = N/domain.thread_size +1;
        //if(count<500)  
        //cout<<count<<"\tGpu: "<<milliseconds<<"\tCPU :"<<elapsed_seconds_cpu.count()*1000<<"\tCPU_for :"<<elapsed_seconds_cpu_for.count()*1000<< "\tdist :"<<elapsed_seconds_dist.count()*1000<<"\tup :"<<elapsed_seconds_up.count()*1000<<"\tcpu_count:"<<cpu_count<<"\tGPU_count:"<<gpu_count<< "\t cpu_x :"<<domain.cpu_x<<endl;
        file1<<count<<"\tGpu: "<<milliseconds<<"\tCPU :"<<elapsed_seconds_cpu.count()*1000<<"\tCPU_for :"<<elapsed_seconds_cpu_for.count()*1000<< "\tdist :"<<elapsed_seconds_dist.count()*1000<<"\tup :"<<elapsed_seconds_up.count()*1000<< "\t cpu_x :"<<domain.cpu_x<<endl;
        t = t+timestep_length;
        count++;
    }
    ...
    std::cout << "elapsed time MemCpy" << elapsed_seconds.count() << "s\n";
    file1<<"elapsed time MemCpy" << elapsed_seconds.count() << "s\n";
    string vtk_name = part_out_name_base+"/Results"+ ".txt";
    fstream file;
    file.open(vtk_name,std::ios_base::app);
    file<<"MemCpy :"<<elapsed_seconds.count() << "\t"<<domain.cpu_x<<"\n";
    file.close();
    file1.close();
}
