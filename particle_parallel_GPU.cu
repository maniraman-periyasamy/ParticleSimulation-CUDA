...
void input_para(string name)
{
    ...
    string subName = part_out_name_base+ "/Particle_parallel3_";
...
template <typename T>
__global__ void main_kernel_force (particle<T> *elements, T delta_t, T epsilon, T sigma, int num_par, Domain<T> domain, int * cell_arr )
{
    int id = threadIdx.x + blockIdx.x * blockDim.x;
...
{
    string vtk_name = part_out_name_base+"/Particle_parallel3_/"+vtk_out_name_base + "_" + to_string(vtk_count) + ".vtk";
    fstream file;
...
{
    string out_name = part_out_name_base+"/Particle_parallel3_/"+part_out_name_base + "_" + to_string(out_count) + ".out";
    fstream file;
...
int main( int argc, char *argv[] )
{
    ...
    string result_name = part_out_name_base+"/Particle_parallel5_/result_time.txt";
    fstream file1;
  ...
  while(count <= int(std::round(time_end/timestep_length)))
    {
     ...
     // ************************ write out vtk and out **********************
      ...
      // ************************ Calculate distance **********************
        ...
        init_particle<data_type><<<domain.block_size,domain.thread_size>>>(d_elements,N);
        ...
        update_cell<data_type><<<domain.block_size,domain.thread_size>>>(d_elements,N,domain, d_cell_arr);
        checkError(cudaPeekAtLastError());
        checkError(cudaDeviceSynchronize());
      ...
      // ************************ calculate force **********************  
        ...
        main_kernel_force<data_type><<<domain.block_size,domain.thread_size>>>(d_elements,timestep_length,epsilon,sigma,N,domain,d_cell_arr);
        ...
        //cout<<count<<"\tGpu: "<<milliseconds<< "\tdist :"<<elapsed_seconds_dist.count()*1000<<"\tup :"<<elapsed_seconds_up.count()*1000<<endl;
        file1<<count<<"\tGpu: "<<milliseconds<< "\tdist :"<<elapsed_seconds_dist.count()*1000<<"\tup :"<<elapsed_seconds_up.count()*1000<<endl;
    ...
    /*for(int i =0; i<N; ++i)
    {
        cout<<i<<"  "<<elements[i].force_x<<endl;
    }*/
   
    auto end = std::chrono::system_clock::now();
    std::chrono::duration<double> elapsed_seconds = end-start;
    std::cout << "elapsed time GPU " << elapsed_seconds.count() << "s\n";
    file1<<"elapsed time GPU " << elapsed_seconds.count() << "s\n";
    string vtk_name = part_out_name_base+"/Results"+ ".txt";
    fstream file;
    file.open(vtk_name,std::ios_base::app);
    file<<"GPU : "<<elapsed_seconds.count() << "s\n";
    file.close();
    file1.close();
    checkError(cudaProfilerStop());
